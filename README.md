...
The library synchronously selects up to three lowest-id active user slots and
emits the same message immediately from those clients. This deterministic
redundancy tolerates a sender disappearing without using delayed tamper
detection, retry queues, randomized sender voting, or guard escalation. Those
mechanisms cannot protect a modified map and must not introduce local timer or
RNG behavior into a lockstep game.
